...
ButWin  *butWin_rootCreate(void *packet, ButEnv *env) {
  ButWin  *result;
  int  scrn = DefaultScreen(env->dpy);

  result = create(packet, env, "No Name",
		  DisplayWidth(env->dpy, scrn),
		  DisplayHeight(env->dpy, scrn), NULL);
  result->unmap = NULL;
  result->map = NULL;
  result->destroy = NULL;
  result->quit = NULL;
  result->win = RootWindow(env->dpy, scrn);
  /* VirtualRootWindowOfScreen(DefaultScreenOfDisplay(env->dpy)); */
  result->physWin = result->win;
  result->resized = TRUE;
  result->mapped = TRUE;
  XSelectInput(env->dpy, result->win, ExposureMask);
  butWin_addToTable(result);
  return(result);
}
